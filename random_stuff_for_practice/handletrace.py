...
def cal_avg(list_of_weight, num_client_this_round):
    ...
    for i in range(len(list_of_weight[0])):
        for j in range(1, len(list_of_weight)):
...
current_model = tf.keras.models.load_model("Client/client_model/simple_LSTM_model")
current_model.summary()

initial_w = current_model.get_weights()
train_domain = read_csv("benign.txt", names=['domain'], nrows=10000)
